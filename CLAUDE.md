# AI Session Governance — Client Brain: marmareos

You are operating inside the private brain of ONE client (`marmareos`). Any AI session
...
1. **Single tenant.** Everything you read or write here concerns `marmareos` only.
...
2. **Task state lives in NXTO, never here.** `50-tasks/tasks.md` is a generated,
   ...
   `client_id: marmareos`. Do not maintain task lists in Markdown.
